...
import { ArrowLeft, ArrowRight, PenLine } from 'lucide-vue-next'
...
async function start() {
  ...
  try {
    const session = await api.createFillTraining({
    ...
    error.value = e instanceof Error ? e.message : '概念记忆训练创建失败，请重试'
  } finally {
...
onMounted(async () => {
  try {
    catalog.value = await api.fillTrainingCatalog()
...
<template>
  <main class="min-h-dvh px-5 pb-10 pt-[max(1.25rem,env(safe-area-inset-top))]">
    <header class="flex items-center gap-4"><button class="grid h-11 w-11 place-items-center rounded-full bg-white shadow-sm" aria-label="返回" @click="router.push('/')"><ArrowLeft :size="20"/></button><div><h1 class="text-xl font-bold">概念记忆 · 知识填空</h1><p class="mt-1 text-xs text-slate-400">专门练习概念、定义与公式</p></div></header>
    <LoadingState v-if="loading" class="mt-20"/>
    <section v-else-if="catalog?.subjects.length" class="mt-8 space-y-6">
      <div class="rounded-[1.75rem] bg-ink p-6 text-white"><PenLine :size="25" class="text-[#e9c985]"/><h2 class="mt-5 text-xl font-bold">把概念记得更牢</h2><p class="mt-2 text-sm text-white/60">只出填空题，检查你对知识点的记忆是否准确。</p></div>
      ...
      <div><span class="mb-3 block text-sm font-semibold">题量</span><div class="flex flex-wrap gap-2"><button v-for="count in countOptions" :key="count" class="min-h-12 min-w-16 flex-1 rounded-xl border px-3 font-semibold" :class="questionCount === count ? 'border-leaf-500 bg-leaf-50 text-leaf-700' : 'border-slate-200 bg-white'" @click="questionCount = count">{{ count }}题</button></div><p class="mt-2 text-xs text-slate-400">当前范围可用 {{ available }} 道填空</p></div>
      <p v-if="error" class="rounded-xl bg-red-50 p-3 text-sm text-red-600">{{ error }}</p>
      <button :disabled="!canCreate || creating" class="flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-leaf-600 font-semibold text-white disabled:bg-slate-200 disabled:text-slate-400" @click="start">{{ creating ? '正在生成…' : '开始填空训练' }}<ArrowRight v-if="!creating" :size="19"/></button>
    </section>
    <section v-else class="mt-16 rounded-2xl bg-white p-6 text-center"><h2 class="font-bold">暂无可用填空题目</h2><p class="mt-2 text-sm text-slate-400">请先导入带学科和章节信息的填空题。</p><p v-if="error" class="mt-4 text-sm text-red-600">{{ error }}</p></section>
  </main>
</template>
