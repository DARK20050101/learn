...
const canSubmit = computed(() => {
  ...
  return type === 'true_false' ? boolAnswer.value !== null : type === 'short_answer' ? !!textAnswer.value.trim() : selected.value.length > 0
})
...
function currentAnswer(): AnswerValue {
  ...
  if (type === 'short_answer') return textAnswer.value.trim()
...
function restoreInput() {
  ...
  else if (item.value?.question.question_type === 'short_answer') textAnswer.value = value
