...
const canSubmit = computed(() => { const t = item.value?.question.question_type; return t === 'true_false' ? boolAnswer.value !== null : (t === 'short_answer' || t === 'fill_blank') ? !!textAnswer.value.trim() : selected.value.length > 0 })
...
function answerValue(): AnswerValue { const t = item.value!.question.question_type; if (t === 'true_false') return boolAnswer.value!; if (t === 'short_answer' || t === 'fill_blank') return textAnswer.value.trim(); return t === 'multiple_choice' ? selected.value : selected.value[0] }
...
function restoreSubmittedAnswer() {
  ...
  else if (item.value?.question.question_type === 'short_answer' || item.value?.question.question_type === 'fill_blank') textAnswer.value = answer
...
<template>
  <main class="min-h-dvh px-5 pb-32 pt-[max(1.25rem,env(safe-area-inset-top))]">
    ...
    <template v-else-if="item">
      ...
      <section v-else-if="item.question.question_type === 'fill_blank'" class="mt-8"><textarea v-model="textAnswer" :disabled="!!result" rows="4" class="w-full resize-none rounded-2xl border border-slate-200 bg-white p-4 text-base leading-7 shadow-sm focus:border-leaf-500 focus:outline-none" placeholder="输入填空处的答案…" /></section>
